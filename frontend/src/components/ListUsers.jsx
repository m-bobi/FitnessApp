...
const ListUsers = () => {
    ...
    useEffect(() => {
        const fetchUsers = async () => {
          try {
            const response = await axios.get("http://localhost:5259/api/User/getAllUsers", {
              headers: {
      ...
      const handleDelete = async (id) => {
        if (window.confirm("Are you sure you want to delete this user?")) {
          try {
            await axios.delete(
              `${config.apiBaseURL}api/User/deleteUser/${id}`,
              {
                headers: {
                  Authorization: `Bearer ${token}`,
                },
                    ...
                    <tbody className="bg-white divide-y divide-gray-200 dark:bg-gray-800 dark:divide-gray-700">
                        {
                            ...
                            allUsers.map(user => (
                                ...
                                <td className="flex items-center p-4 mr-12 space-x-6 whitespace-nowrap">
                                    <img className="w-10 h-10 rounded-full"  src={`/img/users/${user.image}`}  />
                                ...
                                <td className="max-w-sm p-4 overflow-hidden text-base font-normal text-gray-500 truncate xl:max-w-xs dark:text-gray-400">{user.userName}</td>
                                <td className="p-4 text-base font-medium text-gray-900 whitespace-nowrap dark:text-white">{user.name}</td>
                                ...
                                <td className="p-4 space-x-2 whitespace-nowrap">
                                    ...
                                    <button onClick={() => handleDelete(user.id)} data-modal-target="delete-user-modal" data-modal-toggle="delete-user-modal" className="inline-flex items-center px-3 py-2 text-sm font-medium text-center text-white bg-red-600 rounded-lg hover:bg-red-800 focus:ring-4 focus:ring-red-300 dark:focus:ring-red-900">
