...
const OurWorkouts = () => {
    ...
    const fetchWorkouts = async () => {
        try {
          const response = await api.get(`api/Workouts/getAllWorkouts` , setAuthToken(token));
        ...
        <tbody>
         {
            workouts && workouts.length > 0 ? (
                workouts.map((w) => {
                    return (
                        <tr key={w.workoutId} class="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
                ...
                <div>No workouts for the moment!</div>
            )
         }
